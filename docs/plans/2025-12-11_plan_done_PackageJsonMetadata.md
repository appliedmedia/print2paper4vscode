...
All metadata fields listed below are now in `.config/template.package.json` and are produced by `scripts/generate-package-json.mjs`.

The Marketplace publish wave's Lane A audit (linked above) reconciled this plan against the current template: `repository.url` was switched from the vanity domain `https://p2p4vsc.dev` to the canonical GitHub URL `https://github.com/appliedmedia/print2paper4vscode` (so `vsce` can auto-rewrite relative README links), the existing `images/logo-p2p4vsc_128x128px.png` was wired into a new `icon` field, and `license` was normalized from the malformed `SEE LICENSE IN FILE "./LICENSE"` to the SPDX-conformant `SEE LICENSE IN LICENSE`.

`vsce package` now produces a clean 8-file VSIX with zero warnings and zero errors. This plan is closed; further marketplace work tracks under the 2026-04-25 wave plans.
