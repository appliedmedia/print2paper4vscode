...
(0, node_test_1.describe)('Namespace Template Replacement - Auto-Injection', () => {
    let app;
    let utils;
    ...
    (0, node_test_1.it)('should auto-inject ns and ns_ without explicit dictionary entries', () => {
    ...
    (0, node_test_1.it)('should allow callers to override ns/ns_ values if needed', () => {
...
(0, node_test_1.describe)('Namespace Template Replacement - New Convention Verification', () => {
    let app;
    let utils;
    const namespaceDict = {
        ns: 'p2p4vsc',
     ...
     */
    (0, node_test_1.it)('should replace .{{ns_}}menuBtn with .p2p4vsc_menuBtn in CSS', () => {
        const template = '.{{ns_}}menuBtn { color: red; }';
    ...
    (0, node_test_1.it)('should replace multiple namespace occurrences in same line', () => {
        const template = '.{{ns_}}menu .{{ns_}}menuBtn { display: block; }';
        const result = utils.templateDictReplace(template, namespaceDict);
        assert.strictEqual(result, '.p2p4vsc_menu .p2p4vsc_menuBtn { display: block; }');
    });
    (0, node_test_1.it)('should handle {{ns_}} for data attributes with underscore+camelCase convention', () => {
        const template = 'data-{{ns_}}actionType="print"';
        const result = utils.templateDictReplace(template, namespaceDict);
        assert.strictEqual(result, 'data-p2p4vsc_actionType="print"');
    });
});
(0, node_test_1.describe)('Namespace Template Replacement - UIMenu New Convention', () => {
    let app;
    let utils;
    const namespaceDict = {
        ns: 'p2p4vsc',
    ...
    (0, node_test_1.it)('should show what UIMenu.yaml will look like with NEW convention', () => {
...
`;
    ...
    (0, node_test_1.it)('should verify JavaScript querySelector uses NEW convention', () => {
        const jsTemplate = `
...
`;
    ...
    (0, node_test_1.it)('should handle complex CSS selectors with NEW convention', () => {
...
(0, node_test_1.describe)('Namespace Template Replacement - UI.yaml New Convention', () => {
    let app;
    let utils;
    const namespaceDict = {
        ns: 'p2p4vsc',
    ...
    (0, node_test_1.it)('should show what UI.yaml will look like with NEW toolbar convention', () => {
        const refactoredTemplate = `
  #{{ns_}}toolbar {
    position: fixed;
    z-index: 9999;
...
`;
    ...
    (0, node_test_1.it)('should handle zoom controls with NEW convention', () => {
        const template = `
...
(0, node_test_1.describe)('Namespace Template Replacement - UIWebView New Convention', () => {
    let app;
    let utils;
    const namespaceDict = {
        ns: 'p2p4vsc',
    ...
    (0, node_test_1.it)('should verify UIWebView.yaml uses NEW convention for zoom and menu', () => {
        const template = `
  const zoomControls = document.getElementById('{{ns_}}zoomControls');
...
(0, node_test_1.describe)('Namespace Template Replacement - Complex Scenarios with NEW Convention', () => {
    let app;
    let utils;
    const namespaceDict = {
        ns: 'p2p4vsc',
    ...
    (0, node_test_1.it)('should handle HTML with nested elements using NEW convention', () => {
        const template = `
...
`;
    ...
    (0, node_test_1.it)('should handle CSS with pseudo-selectors using NEW convention', () => {
...
.{{ns_}}menuItem::before { content: ''; }`;
    ...
    (0, node_test_1.it)('should verify internal consistency: CSS classes match JS selectors', () => {
    ...
    (0, node_test_1.it)('should handle data attributes with {{ns_}} using underscore+camelCase convention', () => {
        ...
        const result = utils.templateDictReplace(template, namespaceDict);
        assert.strictEqual(result, '<div data-p2p4vsc_menuItemId="123" data-p2p4vsc_actionType="click"></div>');
    });
});
//# sourceMappingURL=Namespace-Template-Replacement.test.js.map