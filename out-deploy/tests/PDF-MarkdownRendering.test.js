 ...
 */
(0, node_test_1.describe)('PDF Markdown HTML Rendering', () => {
    let app;
    let pdf;
    ...
    (0, node_test_1.describe)('renderFromHTML', () => {
        (0, node_test_1.it)('should render simple HTML paragraph', async () => {
            pdf.setupPdf();
        ...
        (0, node_test_1.it)('should render heading elements', async () => {
            pdf.setupPdf();
            const html = '<h1>Heading 1</h1><h2>Heading 2</h2><h3>Heading 3</h3>';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should render inline formatting', async () => {
            pdf.setupPdf();
            const html = '<p>Text with <strong>bold</strong> and <em>italic</em> and <code>code</code></p>';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should render unordered lists', async () => {
            pdf.setupPdf();
            const html = '<ul><li>Item 1</li><li>Item 2</li><li>Item 3</li></ul>';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should render ordered lists', async () => {
            pdf.setupPdf();
            const html = '<ol><li>First</li><li>Second</li><li>Third</li></ol>';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should render code blocks with syntax highlighting', async () => {
            pdf.setupPdf();
            const html = '<pre><code class="language-javascript">const x = 42;\nconsole.log(x);</code></pre>';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should render code blocks without language', async () => {
            pdf.setupPdf();
            const html = '<pre><code>Plain text code\nNo syntax highlighting</code></pre>';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should render blockquotes', async () => {
            pdf.setupPdf();
            const html = '<blockquote><p>This is a quote</p></blockquote>';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should render horizontal rules', async () => {
            pdf.setupPdf();
            const html = '<p>Before</p><hr><p>After</p>';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should handle complex nested structures', async () => {
            pdf.setupPdf();
            const html = `
        <h1>Title</h1>
  ...
  console.log('test');
      ...
      `;
        ...
        (0, node_test_1.it)('should handle empty HTML gracefully', async () => {
            pdf.setupPdf();
            const html = '';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should handle HTML with only whitespace', async () => {
            pdf.setupPdf();
            const html = '   \n\n   ';
            await pdf.renderFromHTML(html);
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
    ...
    (0, node_test_1.describe)('getMarkdownFontInfo', () => {
        (0, node_test_1.it)('should return font info from markdown preview settings', () => {
    ...
    (0, node_test_1.describe)('getFontFromElementStyle', () => {
        (0, node_test_1.it)('should extract font-family from style attribute', () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
        ...
        (0, node_test_1.it)('should extract font-size in pixels', () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
            const mockElement = {
                getAttribute: (attr) => {
        ...
        (0, node_test_1.it)('should extract font-size in points and convert to pixels', () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
            const mockElement = {
                getAttribute: (attr) => {
        ...
        (0, node_test_1.it)('should extract font-size in em and convert to pixels', () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
            const mockElement = {
                getAttribute: (attr) => {
        ...
        (0, node_test_1.it)('should return null for element without style', () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
            const mockElement = {
                getAttribute: () => null
    ...
    (0, node_test_1.describe)('renderTextContent', () => {
        (0, node_test_1.it)('should render plain text', () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
            pdfPrivate.renderTextContent('Simple text');
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should handle empty text', () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
            pdfPrivate.renderTextContent('');
            assert.ok(pdf.docInfo().pdfDoc, 'PDF document should exist');
        });
        (0, node_test_1.it)('should wrap long text', () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
    ...
    (0, node_test_1.describe)('Markdown vs Token rendering integration', () => {
        (0, node_test_1.it)('should generate PDF with markdown rendering mode', async () => {
            ...
            await pdf.generatePdf({ useRenderedMd: true, document: {} });
        ...
        (0, node_test_1.it)('should generate PDF with token rendering mode (raw markdown)', async () => {
            pdf.docInfo().code = '# Heading\n\nParagraph text.';
            pdf.docInfo().languageId = 'markdown';
    ...
    (0, node_test_1.describe)('HTML Element Handlers', () => {
        (0, node_test_1.it)('should have handlers for core required elements', () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
        ...
        (0, node_test_1.it)('should gracefully handle unknown elements', async () => {
            pdf.setupPdf();
    ...
    (0, node_test_1.describe)('Font rendering consistency', () => {
        (0, node_test_1.it)('should use markdown preview font settings', async () => {
            pdf.setupPdf();
        ...
        (0, node_test_1.it)('should override with inline styles when present', async () => {
            pdf.setupPdf();
            const pdfPrivate = pdf;
            const mockElement = {
                getAttribute: (attr) => {
                    if (attr === 'style')
                        return 'font-family: Courier; font-size: 14px';
                    return null;
                }
            };
            const fontInfo = pdfPrivate.getFontFromElementStyle(mockElement);
...
//# sourceMappingURL=PDF-MarkdownRendering.test.js.map