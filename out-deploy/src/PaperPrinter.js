...
exports.PaperPrinter = void 0;
const UIMenu_1 = require("./UIMenu");
const DocInfo_PaperPrinter_1 = require("./DocInfo_PaperPrinter");
...
class PaperPrinter {
    ...
    constructor(args) {
        ...
        this.fn = this.reg.use('vscodeapis.getActiveTextEditor', 'vscodeapis.getEditorTypography', 'vscodeapis.renderMarkdownToHtml', 'tabinspector.detectActiveTabCategory', 'tabinspector.getEditorSelectionOrAll', 'stylize.getThemes', 'os.dictReplace', 'os.getLocale', 'os.fileOpenPrintDialog', 'yaml.create', 'utils.templateDictReplace', 'utils.forceNumber', 'utils.hasContent', 'uimenumgr.getMenuItemIdSelected', 'uimenumgr.getValueForMenuItemId', 'uimenumgr.getValueForMenuItemIdSelected', 'uimenumgr.setValueForPersistIdOnMenuId', 'uimenumgr.getUIMenus', 'uimenumgr.createMenu', 'uimenumgr.addMenu', 'uiwebview.displayPdfPanel', 'pdf.docInfo', 'pdf.readyToPrint', 'pdf.getPageTotal', 'pdf.printWithPreview', 'pdf.printDirectly', 'pdf.saveAsPDF', 'pdf.generatePdf', 'pdf.resetCaches');
        this.dx = this.fn.dx.sub({ name: 'PaperPrinter' });
        // Initialize docInfo
        this._docInfo = DocInfo_PaperPrinter_1.DocInfo_PaperPrinter.create({ reg: this.reg });
    ...
    get lineHeightPx() {
        ...
        const fontSizeId = this.fn.uimenumgr.getMenuItemIdSelected(PaperPrinter_t_1.kFontSizeId.id);
        return parseInt(fontSizeId || PaperPrinter_t_1.kFontSizeId.altId, 10) * editorTypo.sizeToHeightRatio;
    }
    /**
     * Handle print request from UIWebView
     */
    async handlePrintRequest(printType) {
     ...
     */
    async handlePrintCommandFromVSCode() {
        try {
    ...
    async generatePdf() {
        ...
        try {
            ...
            const fontSizeValue = this.fn.uimenumgr.getMenuItemIdSelected(PaperPrinter_t_1.kFontSizeId.id);
            const fontSize = parseInt(fontSizeValue || PaperPrinter_t_1.kFontSizeId.altId, 10);
            dx.out(`PDF GENERATION: Using font size ${fontSize}px`);
            const theme = (this.fn.uimenumgr.getMenuItemIdSelected(PaperPrinter_t_1.kTheme.id) || PaperPrinter_t_1.kTheme.altId);
            // Validate and normalize pageSizeId - use lookup table, fall back to default if invalid
            const rawPageSizeId = this.fn.uimenumgr.getMenuItemIdSelected(PaperPrinter_t_1.kPageSizeId.id);
            const pageSizeId = rawPageSizeId && rawPageSizeId in PaperPrinter_t_1.kPageSizeIdById
                ? rawPageSizeId
                : PaperPrinter_t_1.kPageSizeId.altId;
            // Validate and normalize orient - clamp to valid values, fall back to default if invalid
            const rawOrient = this.fn.uimenumgr.getMenuItemIdSelected(PaperPrinter_t_1.kOrient.id);
            const orient = rawOrient === 'portrait' || rawOrient === 'landscape'
                ? rawOrient
                : PaperPrinter_t_1.kOrient.altId;
            // Validate and normalize marginId - use lookup table, fall back to default if invalid
            const rawMarginId = this.fn.uimenumgr.getMenuItemIdSelected(PaperPrinter_t_1.kMarginId.id);
            const marginId = rawMarginId && rawMarginId in PaperPrinter_t_1.kMarginIdById
                ? rawMarginId
                : PaperPrinter_t_1.kMarginId.altId;
            ...
            const useRenderedMd = this.docInfo().languageId === 'markdown' &&
                !!this.fn.uimenumgr.getValueForMenuItemIdSelected(PaperPrinter_t_1.kMd.id);
    ...
    createMenus() {
        ...
        const menus = PaperPrinter_t_1.kMenus;
        // Build menu configs from constants
        const menuConfigs = menus.map(menuConst => {
            ...
            return {
                ...
                menuItems: this[`menuItems_${methodName}`].bind(this),
                flyoutMenuItemIds: menuConst.flyoutMenuItemIds,
                selectionHandler: ((menuId, menuItemId, contextDict) => {
                    const handler = this[`handleSelection_${methodName}`];
                    return handler.call(this, { menuId, menuItemId });
                }).bind(this),
            };
        });
        // Add header/footer position menus (these don't have constants)
        PaperPrinter_t_1.kHeaderFooterMenuIds.forEach(menuId => {
            const [, pos] = menuId.split('_');
            menuConfigs.push({
                id: menuId,
                displayName: PaperPrinter_t_1.kHeaderFooterMenuItemsById[pos].displayName,
        ...
        menuConfigs.forEach(config => {
            this.dx.out(`Creating menu: ${config.id} with iconSlotTriad: ${JSON.stringify(config.iconSlotTriad)}`);
    ...
    menuItems_Print() {
        return PaperPrinter_t_1.kPrint.menuItems.map(item => ({
            id: item.id,
    ...
    menuItems_Text() {
        ...
        const sizeOptions = PaperPrinter_t_1.kFontSizeId.menuItems.map(item => ({
            id: item.id,
            displayName: item.displayName,
            iconSlotTriad: { begin: '', main: '', end: '' },
        }));
        ...
        dx.out(`Generated ${sizeOptions.length} menu items: ${sizeOptions.map(item => item.id).join(', ')}`);
        dx.done();
        return sizeOptions;
    }
    menuItems_Page() {
        // Header and Footer menus (which have their own flyouts) - now first
        const headerFooterMenus = [PaperPrinter_t_1.kHeader.id, PaperPrinter_t_1.kFooter.id].map(id => ({
            id: id,
            displayName: id === PaperPrinter_t_1.kHeader.id ? PaperPrinter_t_1.kHeader.displayName : PaperPrinter_t_1.kFooter.displayName,
            iconSlotTriad: { begin: '', main: '', end: '' },
        }));
        // Page submenu references (Size, Orient, Margin)
        const pageSubmenus = [PaperPrinter_t_1.kPageSizeId.id, PaperPrinter_t_1.kOrient.id, PaperPrinter_t_1.kMarginId.id].map(id => {
            if (id === PaperPrinter_t_1.kPageSizeId.id)
                return {
                    id: id,
                    displayName: PaperPrinter_t_1.kPageSizeId.displayName,
                    iconSlotTriad: { begin: '', main: '', end: '' },
                };
            if (id === PaperPrinter_t_1.kOrient.id)
                return {
                    id: id,
                    displayName: PaperPrinter_t_1.kOrient.displayName,
                    iconSlotTriad: { begin: '', main: '', end: '' },
                };
            return {
                id: id,
                displayName: PaperPrinter_t_1.kMarginId.displayName,
                iconSlotTriad: { begin: '', main: '', end: '' },
            };
        });
        return [...headerFooterMenus, ...pageSubmenus];
    }
    menuItems_PageSizeId() {
        // Use centralized const - single source of truth for page sizes
        return PaperPrinter_t_1.kPageSizeId.menuItems.map(item => ({
            id: item.id,
            displayName: item.displayName,
            iconSlotTriad: { begin: '', main: '', end: '' },
        }));
    ...
    menuItems_Orient() {
        ...
        return PaperPrinter_t_1.kOrient.menuItems.map(item => ({
            id: item.id,
    ...
    menuItems_MarginId() {
        // Use centralized const with template replacement for SVG icons
        return PaperPrinter_t_1.kMarginId.menuItems.map(item => ({
            id: item.id,
            displayName: this.fn.utils.templateDictReplace(item.displayName, this.yaml()),
            iconSlotTriad: { begin: '', main: '', end: '' },
        }));
    ...
    menuItems_Header() {
        ...
        return PaperPrinter_t_1.kHeaderFooter.menuItems.map(item => ({
            id: `header_${item.id}`,
            displayName: item.displayName,
            iconSlotTriad: { begin: '', main: '', end: '' },
        }));
    ...
    menuItems_ZoomLevel() {
        ...
        return PaperPrinter_t_1.kZoomLevel.menuItems.map(item => {
    ...
    menuItems_Md() {
        ...
        return PaperPrinter_t_1.kMd.menuItems.map(item => ({
            id: item.id,
            displayName: item.displayName,
            iconSlotTriad: { begin: '', main: '', end: '' },
        }));
     ...
     */
    async regenerateAndUpdateWebview() {
    ...
    async handleSelection_Header() {
        // Header menu doesn't have direct selections, just opens flyouts
        return { id: Persist_1.kEmptyNoPersist, value: Persist_1.kEmptyNoPersist };
    }
    async handleSelection_Footer() {
        // Footer menu doesn't have direct selections, just opens flyouts
        return { id: Persist_1.kEmptyNoPersist, value: Persist_1.kEmptyNoPersist };
    }
    async handleSelection_HeaderFooter(args) {
        ...
        if (menuId.startsWith('header_') || menuId.startsWith('footer_')) {
            ...
            if (menuItemId === UIMenu_1.UIMenu.defaultId()) {
                ...
                if (menuId === 'header_middle') {
                    ...
                    id = PaperPrinter_t_1.kHeaderFooter.none;
                }
                value = id;
            }
            else {
                // Get current value from persist
                const currentValue = this.fn.uimenumgr.getMenuItemIdSelected(menuId) || PaperPrinter_t_1.kHeaderFooter.none;
                // Toggle behavior: if clicking the same item that's already selected, deselect it (set to 'none')
                if (currentValue === menuItemId) {
                    id = PaperPrinter_t_1.kHeaderFooter.none;
                    value = PaperPrinter_t_1.kHeaderFooter.none;
                }
                else {
    ...
    async handleSelection_Theme(args) {
        const dx = this.dx.sub({ name: 'handleSelection_Theme' });
        dx.require(args, ['menuId', 'menuItemId']);
        const { menuId, menuItemId } = args;
        let id = menuItemId;
        let value = id; // value is the theme ID
        if (menuItemId === UIMenu_1.UIMenu.defaultId()) {
            ...
            const fallbackTheme = availableThemes[0]?.id || PaperPrinter_t_1.kTheme.altId;
            ...
            this.fn.uimenumgr.setValueForPersistIdOnMenuId({
                menuId: PaperPrinter_t_1.kTheme.id,
                persistId: PaperPrinter_t_1.kTheme.id,
                value: menuItemId
            });
    ...
    async handleSelection_Text(args) {
        ...
        let value = id; // In this case, the value is the fontSizePx which happens to be the id
        if (menuItemId === UIMenu_1.UIMenu.defaultId()) {
            ...
            this.fn.uimenumgr.setValueForPersistIdOnMenuId({
                menuId: PaperPrinter_t_1.kFontSizeId.id,
                persistId: PaperPrinter_t_1.kFontSizeId.id,
                value: menuItemId
            });
            void this.regenerateAndUpdateWebview();
        }
    ...
    async handleSelection_PageSizeId(args) {
        ...
        if (menuItemId === UIMenu_1.UIMenu.defaultId()) {
            ...
            this.fn.uimenumgr.setValueForPersistIdOnMenuId({
                menuId: PaperPrinter_t_1.kPageSizeId.id,
                persistId: PaperPrinter_t_1.kPageSizeId.id,
                value: menuItemId
            });
            void this.regenerateAndUpdateWebview();
        }
    ...
    async handleSelection_Orient(args) {
        const dx = this.dx.sub({ name: 'handleSelection_Orient' });
        dx.require(args, ['menuId', 'menuItemId']);
        const { menuId, menuItemId } = args;
        let id = menuItemId;
        let value = id; // value is the orient ID
        if (menuItemId === UIMenu_1.UIMenu.defaultId()) {
            ...
            // Update orientation
            this.fn.uimenumgr.setValueForPersistIdOnMenuId({
                menuId: PaperPrinter_t_1.kOrient.id,
                persistId: PaperPrinter_t_1.kOrient.id,
                value: menuItemId
            });
            void this.regenerateAndUpdateWebview();
        }
    ...
    async handleSelection_MarginId(args) {
        const dx = this.dx.sub({ name: 'handleSelection_MarginId' });
        dx.require(args, ['menuId', 'menuItemId']);
        const { menuId, menuItemId } = args;
        const defaultMarginId = 'normal';
        let id = menuItemId;
        let value = id; // value is the margin ID
        if (menuItemId === UIMenu_1.UIMenu.defaultId()) {
            ...
            this.fn.uimenumgr.setValueForPersistIdOnMenuId({
                menuId: PaperPrinter_t_1.kMarginId.id,
                persistId: PaperPrinter_t_1.kMarginId.id,
                value: menuItemId
            });
            void this.regenerateAndUpdateWebview();
        }
     ...
     */
    zoomLevel_setTextEdit(zoomValue) {
        const dx = this.dx.sub({ name: 'zoomLevel_setTextEdit' });
        if (this.fn.utils.hasContent(zoomValue)) {
            const menuId = PaperPrinter_t_1.kZoomLevel.id;
            const triadMain = PaperPrinter_t_1.kZoomLevel.iconSlotTriad.main;
            const persistId = triadMain.persistId;
            if (persistId) {
                const persistValue = this.fn.utils.forceNumber(zoomValue);
     ...
     */
    async handleSelection_ZoomLevel(menuId, menuItemId, contextDict) {
        const dx = this.dx.sub({ name: 'handleSelection_ZoomLevel' });
        dx.out(`ZoomLevel selection: menuItemId=${menuItemId}`);
        let id = menuItemId;
        let value = this.fn.uimenumgr.getValueForMenuItemId({ menuId, menuItemId });
        if (menuItemId === UIMenu_1.UIMenu.defaultId()) {
     ...
     */
    async handleSelection_Md(args) {
        const dx = this.dx.sub({ name: 'handleSelection_Md' });
        dx.require(args, ['menuId', 'menuItemId']);
        const { menuId, menuItemId } = args;
        const id = menuItemId === UIMenu_1.UIMenu.defaultId() ? PaperPrinter_t_1.kMd_Raw.id : menuItemId;
     ...
     */
    async handleSelection_ZoomInOut(args) {
        ...
        if (menuItemId !== UIMenu_1.UIMenu.defaultId()) {
            ...
            let direction;
            if (menuId === PaperPrinter_t_1.kZoomOut.id) {
                direction = -1;
            }
            else if (menuId === PaperPrinter_t_1.kZoomIn.id) {
                direction = +1;
            }
            else {
                dx.error(`Unknown menuId: ${menuId}. Expected ${PaperPrinter_t_1.kZoomOut.id} or ${PaperPrinter_t_1.kZoomIn.id}`);
                dx.done();
                return { id, value };
            }
            // Get current zoom value with proper validation
            const rawZoom = this.fn.uimenumgr.getValueForMenuItemIdSelected(PaperPrinter_t_1.kZoomLevel.id);
            ...
            if (Number.isNaN(currentZoom) || currentZoom === 0) {
                dx.out(`Invalid currentZoom (${rawZoom}), using altValue: ${PaperPrinter_t_1.kZoomLevel.altValue}`);
                const fallbackZoom = Number(PaperPrinter_t_1.kZoomLevel.altValue);
                if (Number.isNaN(fallbackZoom)) {
                    dx.error(`altValue is also invalid: ${PaperPrinter_t_1.kZoomLevel.altValue}`);
                    dx.done();
                    return { id, value };
                }
            }
            const validCurrentZoom = Number.isNaN(currentZoom) || currentZoom === 0
                ? Number(PaperPrinter_t_1.kZoomLevel.altValue)
                : currentZoom;
            ...
            const adjustment = direction * PaperPrinter_t_1.kZoomLevel.stepAmount;
            const newZoom = Math.max(PaperPrinter_t_1.kZoomLevel.min, Math.min(PaperPrinter_t_1.kZoomLevel.max, Math.round((validCurrentZoom + adjustment) * 100) / 100));
            dx.out(`${menuId}: newZoom=${newZoom}`);
            // Persist the new zoom level (custom value - menuItemId = menuId)
            this.fn.uimenumgr.setValueForPersistIdOnMenuId({
                menuId: PaperPrinter_t_1.kZoomLevel.id,
                persistId: PaperPrinter_t_1.kZoomLevel.id,
                value: PaperPrinter_t_1.kZoomLevel.id
            });
...
exports.PaperPrinter = PaperPrinter;
// end, PaperPrinter.ts
//# sourceMappingURL=PaperPrinter.js.map