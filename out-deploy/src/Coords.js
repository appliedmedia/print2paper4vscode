...
class Coords {
    ...
    static POINTS_PER_INCH = 72;
    static POINTS_PER_MM = 2.834645669;
    static PX_TO_PTS_RATIO = 0.75; // 72 DPI / 96 DPI
    // Base margin constant: 0.4 inch gutter required for all non-full-bleed printers
    static kMarginGutterMinPts = 0.4 * Coords.POINTS_PER_INCH;
    constructor(args) {
        this.reg = args.reg;
     ...
     */
    pdfPtsYToScreenY(pdfPtsY, pageHeightPts) {
     ...
     */
    screenYToPdfPtsY(screenY, pageHeightPts) {
     ...
     */
    getPdfPtsYForTopMarginStart(pageHeightPts, topMarginPts) {
     ...
     */
    getPdfPtsYForBottomMarginStart(bottomMarginPts) {
     ...
     */
    movePdfPtsYDownOneLine(currentPdfPtsY, lineHeightPts) {
     ...
     */
    movePdfPtsYUpOneLine(currentPdfPtsY, lineHeightPts) {
        return currentPdfPtsY + lineHeightPts;
    }
    /**
     ...
     */
    hasPdfPtsYReachedBottomMargin(currentPdfPtsY, bottomMarginPts) {
     ...
     */
    calculatePdfPtsAvailableHeightForContent(pageHeightPts, topMarginPts, bottomMarginPts) {
     ...
     */
    calculatePdfPtsAvailableWidthForContent(pageWidthPts, leftMarginPts, rightMarginPts) {
        return pageWidthPts - leftMarginPts - rightMarginPts;
    }
    /**
     * Debug: Log coordinate information
     */
    debugCoords(pageWidthPts, pageHeightPts, marginsPts, currentPdfY) {
        const dx = this.dx.sub({ name: 'debugCoords' });
        dx.out(`Page: ${pageWidthPts}x${pageHeightPts}pts`);
        dx.out(`Margins: top=${marginsPts.topMarginPts}, bottom=${marginsPts.bottomMarginPts}, left=${marginsPts.leftMarginPts}, right=${marginsPts.rightMarginPts}`);
        dx.out(`Current PDF Y: ${currentPdfY}`);
        dx.out(`Top margin PDF Y: ${this.getPdfPtsYForTopMarginStart(pageHeightPts, marginsPts.topMarginPts)}`);
        dx.out(`Bottom margin PDF Y: ${this.getPdfPtsYForBottomMarginStart(marginsPts.bottomMarginPts)}`);
        dx.out(`Available height: ${this.calculatePdfPtsAvailableHeightForContent(pageHeightPts, marginsPts.topMarginPts, marginsPts.bottomMarginPts)}`);
        dx.out(`Available width: ${this.calculatePdfPtsAvailableWidthForContent(pageWidthPts, marginsPts.leftMarginPts, marginsPts.rightMarginPts)}`);
        dx.done();
    }
}
