...
const DocInfo_PDF_1 = require("./DocInfo_PDF");
...
class PDF {
    ...
    currentY = 0;
    currentLineHeight = 0;
    lastPageBreakMetrics = null;
    // Header/footer styling
    static HEADER_FOOTER_COLOR = '#cccccc'; // Light gray
    // PDF document information (private, accessed via docInfo() method)
    _docInfo;
    constructor(args) {
        this.reg = args.reg;
        this.fn = this.reg.use('ui.showErrorMessage', 'ui.chooseSaveLocation', 'coords.pdfPtsToCssPx', 'coords.cssPxToPdfPts', 'os.dateAsYYYYMMDDHHMMSS', 'os.sanitizeFileName', 'os.fileDelete', 'os.ensureDir', 'os.pathJoin', 'os.fileWrite', 'os.fileOpenPrintDialog', 'os.filePrint', 'os.pathDirname', 'os.fileReveal', 'vscodeapis.getDir_Temp', 'vscodeapis.getEditorTypography', 'vscodeapis.getConfiguration', 'stylize.tokenize', 'yaml.create', 'utils.templateDictReplace', 'utils.hasContent', 'uimenumgr.getMenuItemIdSelected');
        this.dx = this.fn.dx.sub({ name: 'PDF' });
        this._docInfo = DocInfo_PDF_1.DocInfo_PDF.create({ reg: this.reg });
    ...
    async printWithPreview(descriptiveName) {
    ...
    async printDirectly(descriptiveName) {
    ...
    async saveAsPDF(descriptiveName) {
    ...
    mapFontFamilyToJsPDF(fontFamily, doc) {
     ...
     */
    pageSizeToPts(width, height, unit) {
     ...
     */
    setTextColorFromWebColor(doc, color) {
    ...
    getUnitForPageSize(pageSize) {
        // Get unit from centralized constant (single source of truth)
        const unit = PaperPrinter_t_1.kPageSizeIdById[pageSize].unit;
    ...
    getPageDimensions(pageSize, orient) {
        // Get dimensions from centralized const object (id ? metadata)
        const metadata = PaperPrinter_t_1.kPageSizeIdById[pageSize];
    ...
    async getPageSizePx() {
        ...
        try {
            ...
            const menuKeys = ['pageSizeId', 'orient'];
            const selections = {};
            for (const key of menuKeys) {
                selections[key] = this.fn.uimenumgr.getMenuItemIdSelected(key);
            }
            const pageSizeId = selections.pageSizeId;
            const orient = selections.orient;
    ...
    setupPdf() {
        ...
        try {
            ...
            this.docInfo().pdfDoc = new jspdf_1.default({
            ...
            dx.out(`Initial position: (${this.currentX}, ${this.currentY}) (leftMargin=${marginsPts.leftMarginPts}, topMargin=${marginsPts.topMarginPts})`);
            ...
            dx.out(`PDF document setup: ${this.docInfo().pageSizeId} ${this.docInfo().orient}, ${fontSizePts}pt font, ${this.currentLineHeight}pt line height`);
            dx.out(`Page dimensions: ${pageWidthPts}x${pageHeightPts}pts, margins: top=${marginsPts.topMarginPts}, bottom=${marginsPts.bottomMarginPts}, left=${marginsPts.leftMarginPts}, right=${marginsPts.rightMarginPts}`);
     ...
     */
    findCharacterBreakPoint(text, currentXPos, leftMargin, availableWidth) {
        if (!this.docInfo().pdfDoc) {
            return text.length;
        }
    ...
    renderFromTokens(tokens) {
        ...
        try {
            ...
            for (let lineNumber = 0; lineNumber < tokens.length; lineNumber++) {
                ...
                for (const token of lineTokens) {
                    ...
                    if (content) {
                        ...
                        while (content.length > 0) {
                            ...
                            const charsToRender = this.findCharacterBreakPoint(content, xPos, marginsPts.leftMarginPts, availableWidth);
                            if (charsToRender === 0) {
                                ...
                                if (this.shouldBreakPage(yPos)) {
                                    ...
                                    dx.out(`Page break during wrapping at line ${lineNumber}: yPos=${yPos} > bottomMargin=${bottomMargin}`);
                                    this.addPageBreak();
                                    yPos = this.currentY;
                                    xPos = this.currentX;
                                }
                            ...
                            if (lineNumber < 3) {
                                dx.out(`First few lines - Line ${lineNumber}, Token portion: "${portionToRender}", Position: (${xPos}, ${yPos})`);
                            }
                ...
                this.currentX = marginsPts.leftMarginPts;
                dx.out(`Line ${lineNumber} complete: Y moved from ${oldY} to ${this.currentY} (down by ${this.currentLineHeight})`);
                if (this.shouldBreakPage(this.currentY)) {
                    const bottomMargin = this.lastPageBreakMetrics?.bottomMarginY ?? 0;
                    dx.out(`Page break at line ${lineNumber}: currentY=${this.currentY} > bottomMargin=${bottomMargin}`);
                    this.addPageBreak();
    ...
    getMarkdownFontInfo() {
        ...
        const mdFontFamily = mdConfig.get('preview.fontFamily');
        const mdFontSize = mdConfig.get('preview.fontSize');
     ...
     */
    getFontFromElementStyle(element) {
    ...
    async renderFromHTML(html) {
        ...
        try {
            ...
            for (const element of root.childNodes) {
                if (element.nodeType === node_html_parser_1.NodeType.ELEMENT_NODE) {
                    await this.renderHTMLElement(element);
                }
            }
     ...
     * }
     */
    async renderHTMLElement(element) {
    ...
    renderInlineContent(element) {
        ...
        for (const child of element.childNodes) {
            ...
            else if (child.nodeType === node_html_parser_1.NodeType.ELEMENT_NODE) {
                const el = child;
    ...
    renderTextContent(text) {
        ...
        while (content.length > 0) {
            ...
            const charsToRender = this.findCharacterBreakPoint(content, this.currentX, marginsPts.leftMarginPts, availableWidth);
            if (charsToRender === 0) {
                // Wrap to next line
                this.currentY += this.currentLineHeight;
                this.currentX = marginsPts.leftMarginPts;
    ...
    async renderCodeBlock(element) {
        ...
        const langClass = classAttr.split(' ').find((c) => c.startsWith('language-'));
     ...
     */
    async renderBlockquote(element) {
        // Save left margin and indent
        const savedLeftMargin = this.docInfo().marginPts.leftMarginPts;
        this.docInfo().marginPts.leftMarginPts += 20;
        this.currentX = this.docInfo().marginPts.leftMarginPts;
        ...
        for (const child of element.childNodes) {
            if (child.nodeType === node_html_parser_1.NodeType.ELEMENT_NODE) {
                await this.renderHTMLElement(child);
            }
        }
        // Restore margin
        this.docInfo().marginPts.leftMarginPts = savedLeftMargin;
    ...
    renderHorizontalRule() {
        ...
        doc.line(marginsPts.leftMarginPts, this.currentY, marginsPts.leftMarginPts + availableWidth, this.currentY);
        this.currentY += 6; // Spacing after
    }
    computePageBreakMetrics() {
        const pageSize = this.getPageDimensions(this.docInfo().pageSizeId, this.docInfo().orient);
        const unit = this.getUnitForPageSize(this.docInfo().pageSizeId);
    ...
    addHeaderAndFooter() {
        ...
        const formatContent = (element, position) => {
            if (!element || element === PaperPrinter_t_1.kHeaderFooter.none)
                return null;
            // Validate element is a valid key in kHeaderFooterSubmenuById
            if (!(element in PaperPrinter_t_1.kHeaderFooterSubmenuById)) {
                this.dx.error(`Invalid header/footer element: ${element}`);
                return null;
            }
            const template = PaperPrinter_t_1.kHeaderFooterSubmenuById[element].template;
            const templateDict = {
                title: docTitle,
                '#': String(pageNumber),
     ...
     */
    renderHeaderFooterElements(elements, y, widthPts, margins) {
        if (!this.docInfo().pdfDoc) {
            return;
        }
    ...
    finishPdf() {
        ...
        try {
            ...
            dx.out(`PDF FINALIZED: ${this.docInfo().pageTotal} pages with ${this.docInfo().fontSizePx}px font`);
        }
        catch (error) {
